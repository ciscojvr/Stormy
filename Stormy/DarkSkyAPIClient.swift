...
//    let downloader = JSONDownloader()
    ...
    let decoder = JSONDecoder() //this type is part of this Codable functionality and will handle most of the decoding work for us, if not all. When we task JSONDecoder with parsing, we don't even have to do the work of converting from a data type that we get back from the network request to a dictionary representation that we did with JSON serialization. All of that is handled for us, so we we don't need our custom type JSONDownloader from above.
            ...
            DispatchQueue.main.async {
                if let data = data {
                    ...
                    if httpResponse.statusCode == 200 {
                        do {
                            let weather = try self.decoder.decode(Weather.self, from: data) // Everything that we had implemented with our own solution, where we used a failable initializer, and we parsed each key, and cast that to each value type, all of that has been replaced with this single line of code.
                        ...
                        // we can now completely JSONDownloader altogether because we've replicated that entire functionality over here.
                    }
...
//        // Below code no longer needed after using decoder.
